...
PlotEnsembleError <- function(object, dt.format='as.POSIXct') {
  ...
  title.name <-
    "Percent error for the forecast model ensemble across time"
...
PlotTrainingActualAndForecast <- function(object,
  ...
  if (!short.term) {
    ...
    dtbreaks <- seq(min(training$dt), max(forecast$dt), by=paste(freq, dt.units))
  }
  ...
  title.name <- paste("Training and forecast", freq.tmp)

  p <- ggplot2::ggplot(data=all, ggplot2::aes(x=dt, y=value, color=type))
  p <- p + ggplot2::geom_ribbon(data=forecast,
                                ggplot2::aes(x=dt, y=NULL,
  ...
  p <- p + ggplot2::geom_ribbon(data=forecast,
                                ggplot2::aes(x=dt, y=NULL,
                                             ...
                                             color='red'),
                                fill="red", alpha=0.3, color='transparent',
                                size=line.width)
  p <- p + ggplot2::geom_line(size=line.width, alpha=1.5)
  if (!is.null(evaluation)) {
  ...
  p <- p +
    ggplot2::scale_color_manual(values=scales::hue_pal()(3)[color.idx],
                                name="") +
    ggplot2::scale_x_datetime(breaks=dtbreaks,
                              labels=scales::date_format("%Y-%m-%d")) +

    ggplot2::ylim(c(0.9 * min(training$value),
