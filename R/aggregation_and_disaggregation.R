...
#' @param fit.season.terms: whether to fit seasonality terms for changes in the
#'   day of week proportions due to seasonality
#' @return day of week proportions for the forecast dts based on bootstrap
#'   sample
#' @noRd
...
#'  regression
#'
#' @param daily.history: dataframe containing fields for dt, actual,
...
DisaggregateForecast <- function(unaggregated.history,
  ...
  aggr.totals.range.lower <- rep(aggregated.forecast$range.forecast.lower,
                                 each=disaggregate.period)
  aggr.totals.range.upper <- rep(aggregated.forecast$range.forecast.upper,
                                 each=disaggregate.period)

  aggr.totals.uncertainty.lower <-
    rep(aggregated.forecast$range.uncertainty.lower,
        each=disaggregate.period)
  aggr.totals.uncertainty.upper <-
    rep(aggregated.forecast$range.uncertainty.upper,
        each=disaggregate.period)
  ...
  forecast.disaggr.seq <- seq((max(unaggregated.history$dt) + dt_units(1)),
                               max(aggregated.forecast$dt), by=dt.unit)
  if (!is.na(disaggregate.format)) {
    format.disaggr <- get(names(disaggregate.format))
    disaggr.inst.train <- format.disaggr(unaggregated.history$dt,
                                         disaggregate.format)
    ...
    disaggr.inst.forecast <- rep(1:disaggregate.period,
                                 ceiling((length(forecast.disaggr.seq) /
                                         disaggregate.period)))
    disaggr.inst.forecast <-
      disaggr.inst.forecast[1:length(forecast.disaggr.seq)]
  ...
  disaggregated.forecast <-
    data.frame(dt=forecast.disaggr.seq,
               forecast=disaggr.predict * aggr.totals.forecast,
               forecast.lower=disaggr.predict.lower *
                 aggr.totals.forecast.lower,
               forecast.upper= disaggr.predict.upper *
                 aggr.totals.forecast.upper,
               range.forecast.lower=disaggr.predict *
                 aggr.totals.range.lower,
               range.forecast.upper=disaggr.predict *
                 aggr.totals.range.upper,
               range.uncertainty.lower=disaggr.predict.lower *
                 aggr.totals.uncertainty.lower,
               range.uncertainty.upper=disaggr.predict.upper *
                 aggr.totals.uncertainty.upper)
  ...
  if (get.pred.int) {
    ...
    disaggregated.forecast$range.uncertainty.lower <-
      disaggr.pi$prop.lower * aggr.totals.uncertainty.lower
    disaggregated.forecast$range.uncertainty.upper <-
      disaggr.pi$prop.upper * aggr.totals.uncertainty.upper
  }
